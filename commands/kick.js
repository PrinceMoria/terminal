...
module.exports.run = async (bot, message, args) => {
    ...
    if (!tokick) return message.channel.send("`/" + message.guild + "/" + message.channel + "/`" + ":x: Couldn't find user.");
    if (tokick.hasPermission("MANAGE_MESSAGES")) return message.channel.send("`/" + message.guild + "/" + message.channel + "/`" + ":x: The user you are trying to kick is either the same, or higher ranking than you.");
    var member = message.mentions.members.first();
    member.kick().then((member) => {
        message.channel.send("`/" + message.guild + "/" + message.channel + "/`" + ":white_check_mark: " + member.displayName + " has been kicked by " + message.author.username);
    })
}
