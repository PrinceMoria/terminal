...
module.exports.run = async (bot, message, args) => {
  ...
  if(!rMember.roles.has(gRole.id)) return message.reply("`/" + message.guild + "/" + message.channel + "/`" + `:x: That person hasn't got that role.`);
  await(rMember.removeRole(gRole.id));

  try{
    await message.channel.send("`/" + message.guild + "/" + message.channel + "/`" + `:white_check_mark: Removed ${rMember} from ${gRole.name}.`)
  }catch(e){
  }
}
module.exports.help = {
